...
public class Grapple : MonoBehaviour
{
    ...
    public float grapplerSpd = 10;
    public float grappleLength = 7;
    public float grappleInLength = 0.5f;
    public int unsafeTileHealthPenality = 2;
    ...
    void FixedUpdate()
    {
        ...
        {
            case eMode.gOut:
                p1 += directions[facing] * grapplerSpd * Time.fixedDeltaTime;
                grapHead.transform.position = p1;
                grapLine.SetPosition(1, p1);
                ...
                break;
            case eMode.gInMiss:
                p1 -= directions[facing] * 2 * grapplerSpd * Time.fixedDeltaTime;
                ...
                break;
            case eMode.gInHit:
                float dist = grappleInLength + grapplerSpd * Time.fixedDeltaTime;
                ...
                p0 += directions[facing] * grapplerSpd * Time.fixedDeltaTime;
                transform.position = p0;
                grapLine.SetPosition(0, p0);
                grapHead.transform.position = p1;
                break;
    ...
    void StopGrapple()
    {
        ...
        {
            dray.ResetInRoom(unsafeTileHealthPenality);

        }
        grapHead.SetActive(false);
        mode = eMode.none;
    }
    void OnColliderEnter(Collider colld)
    {
